...
class Clientes
{
    public static function nuevo($razonSocial, $telefono, $email)
    {
        $bd = BD::obtener();
        $sentencia = $bd->prepare("insert into clientes(razonSocial, telefono, email, idUsuario) VALUES (?, ?, ?, ?);");
        return $sentencia->execute([$razonSocial, $telefono, $email, SesionService::obtenerIdUsuarioLogueado()]);
    }
    ...
    {
        $bd = BD::obtener();
        $sentencia = $bd->prepare("select id, razonSocial, telefono, email from clientes where idUsuario = ?;");
    ...
    {
        $bd = BD::obtener();
        $sentencia = $bd->prepare("select id, razonSocial, telefono, email from clientes where id = ? and idUsuario = ?;");
